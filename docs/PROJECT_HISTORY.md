...
**This is an archive, not current state.** It preserves the chronological
session journal that `CURRENT_WORK.md` accumulated between 2026-07-23 and
2026-09-01, moved here verbatim during the shared-context V2 migration so the
handoff file could become readable.

Nothing here should be treated as describing the live system. Read instead:

| For | Read |
|---|---|
| current state and the next step | `CURRENT_WORK.md` |
| durable decisions and their rationale | `DECISIONS.md` |
| what is deployed, and restore paths | `docs/DEPLOY.md` |
| how the system works | `docs/ARCHITECTURE.md` |
| where things live | `docs/REPO_MAP.md` |

Entries are newest-first, as they were written. Some claims were accurate only
on their date — most notably the proof-script count, which grew from sixteen to
forty, and the pre-submission handoff instructions, which the hackathon's close
on 2026-08-12 obsoleted.
