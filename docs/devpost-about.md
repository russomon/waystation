...
- **Provable provenance** — SHA-256 manifest of every input and derivative
  (following Genblaze's provenance-manifest model), written to B2 under
  **Object Lock COMPLIANCE** — in our live run the bucket owner's own key,
  holding `deleteFiles` + `bypassGovernance`, was refused deletion.
...
Genblaze SDK integration for the manifest layer, AI-targeted escalation
(vision review at the exact timecodes the deterministic lane flags),
