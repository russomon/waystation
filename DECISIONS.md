...
- Decision: `POST /admin/codes` accepts an optional `code` — 8–64 characters
  after trimming, stored and matched **exactly as written**. Case-sensitive
  was the user's call, made the same day, over an earlier case-insensitive
  draft: the admin says how the code is capitalised on the call rather than
  the gateway guessing. A chosen code is refused (409) if it exactly matches
  any active code, including the admin's, because login takes the first
  match and a shared code would credit one client's transfers to another.
  That refusal is not an oracle: only the admin session can reach the route,
  and the admin issued every code. Because chosen codes carry less entropy, `/session`
...
- Verified: `scripts/access-codes-proof.sh` — 7 characters refused, the
  admin's own code refused, trimmed and kept as written, exact duplicate
  refused, login exactly as written, other case / near-miss /
  lower-cased-generated refused, 90 attempts from 30 addresses tripping the
  global cap; five mutations caught.
